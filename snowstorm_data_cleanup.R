...
snow_data <- do.call("rbind",lapply(filenames,FUN=function(files){ read.csv(files)}))
...
snow_clean$BEGIN_DATE_TIME <- format(as.POSIXct(snow_clean$BEGIN_DATE_TIME,format='%d-%b-%y %H:%M:%S'),format='%m/%d/%Y')
snow_clean$END_DATE_TIME <- format(as.POSIXct(snow_clean$END_DATE_TIME,format='%d-%b-%y %H:%M:%S'),format='%m/%d/%Y')
